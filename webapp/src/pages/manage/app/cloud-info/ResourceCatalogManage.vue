    ...
    <div class="investment">
        ...
        <Form ref="formInline" :model="formInline" :rules="ruleInline" inline>
            ...
            <FormItem prop="network">
                <Select v-model="formInline.sysMaintenance" placeholder="运维状况" clearable style="width: 200px;">
...
<script>
    ...
    export default {
        ...
        methods: {
            changePage(destination = this.formInline.currentPage){
                ...
                this.$httpt.get('bigScreenController.do?getResourceCatalogPagination&' +
                    ...
                    'sysMaintenance=' + this.formInline.sysMaintenance+ '&' +
                    ...
                    if(data.success) {
                        ...
                        //     for(let j = 0; j < data.data.list.length; j++) {
                        //         if(this.selectTwo[p] === data.data.list[j].statuss) {
                        //             newarr2[p]++;
                        //         }
                        //     }
                        // }
        ...
        mounted() {
            this.changePage();
            this.$httpt.get('bigScreenController.do?getResourceCatalogPagination').then(({data}) => {
                if(data.success) {
                    let tempOne = [];
                    let tempTwo = [];
                    let tempThree = [];
                    data.data.list.forEach(item => {
                        ...
                        tempTwo.push(item.sysMaintenance);
                        tempThree.push(item.safeguardDept);
                    });
                    this.selectOne = [...new Set(tempOne)];
                    this.selectTwo = [...new Set(tempTwo)];
                    this.selectThree = [...new Set(tempThree)];
                }else {
                    console.error('分页获取表11 信息化系统调研表信息失败')
                }
            }).catch(err => {
